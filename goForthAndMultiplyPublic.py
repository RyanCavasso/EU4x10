    ...
    for file in [f for f in listdir(gameDir + directory) if isfile(join(gameDir + directory, f))]:
        ...
        for line in fin:
            ...
            if skipBlock == 0 and skipStaticBlock == False and re.search(commentRegex, line) == None and match != None and match.group(2) in modifiers:
                ...
                if match.group(2) == "election_cycle":      # if current modifier is election_cycle, multiply by 1.5       
                    fileText += match.group(1) + match.group(2) + " = " + str(round(float(match.group(3)) *  1.5, 2)) + match.group(4) + "\n"
                elif file == "mil.txt" or match.group(2) == "development_cost_modifier" or match.group(2) == "local_development_cost_modifier":     # if current file is for military tech, or if modifier is for base development cost, multiply modifiers by 2
